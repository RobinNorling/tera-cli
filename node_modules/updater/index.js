...
class Updater {
	...
	getRealURL(url, append) {
		let str = ''
		for(const param of url.searchParams) {
			if(param[0] == 'drmkey') {
				const tmp = param[0] + '=' + param[1];
				str = url.href.substr(url.href.indexOf(tmp) - 1, tmp.length + 1)
			}
		}

		const newUrl = new URL(url.toString().replace(str, '') + append);
		newUrl.searchParams.append('drmkey', str.substr(str.indexOf('drmkey=') + 7));
	...
	async downloadFiles(manifest, files) {
		...
		for(const file of files)
			promises.add((async () => {
				log.warn(this.getRealURL(new URL(manifest.url), file));
				const p = this._httpAsync(this.getRealURL(new URL(manifest.url), file))
