...
class webClient {
    constructor(email, password, interfaceAddress) {
        this.email = email;
        this.password = password;
        if(interfaceAddress) {
        	this.axios = axios.create({
            	baseURL: 'https://account.enmasse.com',
	        	httpAgent: new require('http').Agent({ localAddress: interfaceAddress }),
				httpsAgent: new require('https').Agent({ localAddress: interfaceAddress })
        	});
        } else {
        	this.axios = axios.create({
            	baseURL: 'https://account.enmasse.com'
    ...
    async getLogin(){
        ...
        if(!accountInfo || !accountInfo.info || !accountInfo.info.custom || !accountInfo.info.custom[0].id) {
        	return {};
        }
        // Get Auth Token
