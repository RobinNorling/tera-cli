	...
	async updateMod(name) {
		...
		if(github) {
			...
			if(pkg._compatInfo.drmKey) {
				const tempManifestURL = new URL(manifestUrl);
				tempManifestURL.searchParams.append('drmkey', pkg._compatInfo.drmKey);
				manifestUrl = tempManifestURL.toString();

				const tempDefaultURL = new URL(defaultUrl);
				tempDefaultURL.searchParams.append('drmkey', pkg._compatInfo.drmKey);
				defaultUrl = tempDefaultURL.toString();
			}
		}
		else {
			manifestUrl = url
