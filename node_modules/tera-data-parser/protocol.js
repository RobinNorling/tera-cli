...
class TeraProtocol {
	constructor(shareRevision) {
		...
		if(this.majorPatchVersion >= 92) {
			try {
				const integrity = JSON.parse(fs.readFileSync(path.join(teraData, `integrity/${this.majorPatchVersion}.json`), 'utf8'))
				...
				throw Error(`integrity/${this.majorPatchVersion}.json not found`)
			}
		}
	...
	getIntegrity(name) {
		const integrity = (typeof name === 'number' ? this.integrityCode : this.integrityName);
		if(typeof integrity !== 'undefined') {
			return (integrity && integrity.has(name));
		}
